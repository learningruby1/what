    ...
    def generate
      ...
      @number_of_children.times do |i|
        ...
        @children_info[i][:addresses].each_with_index do |address, index|
          table_row [ { :content =>"#{ address[:move_date].strftime("%m/%Y") } to #{ index == 0 ? 'Present' : @children_info[i][:addresses][index-1][:move_date].strftime("%m/%Y") }", :width => 150 }, { :content =>"#{ address[:address] }, #{ address[:city] } #{ address[:state] }#{ address[:country] } #{ address[:zip] }", :width => 170 }, { :content =>"#{ address[:lived_with].upcase }", :width => 120 },                       { :content =>"#{ address[:persone_relationship] }", :width => 100 } ]
        end
      end
      push_table -1, 0
      move_down 40
