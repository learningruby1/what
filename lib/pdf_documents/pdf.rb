module PdfDocument
  class Pdf
    ...
    def generate(document)
      ...
      children_nevada_residency = !(TemplateStep.where(:title => 'Children’s Residency /<spain/>Residencia de los Menores').first.document_answers.where(:document_id => document.id).map(&:answer).include? 'No')
