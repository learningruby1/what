...
field_for_mandatory = current_step.fields.create :field_type => 'radio', :name => 'Home Address /<spain/>Dirección de casa:<option/>Business Address /<spain/>Dirección de Negocio:', :toggle_id => toggle_id, :sub_toggle_id => toggle_id + 1, :mandatory => { :value => /\w+/, :hint => 'Please select one /<spain/>Seleccione uno, por favor' }

toggle_id += 1
current_step.fields.create :name => 'Address /<spain/>Dirección: *', :mandatory => { :value => /^[0-9a-zA-Z\-,.\/ #]+$/, :hint => 'Please enter a valid home address /<spain/>Por favor, ponga una dirección de casa o postal válida', :template_field => field_for_mandatory.id, :toggle_option => 'Address' }, :toggle_id => toggle_id, :toggle_option => 'Address'
